...
public static class Networking
{
    ...
    private static void AcceptNewClient(IAsyncResult ar)
    {
        ...
        TcpListener listener = serverTuple.Item2;
        SocketState newState;
        Socket newSocket;
        try
        {
            newSocket = listener.EndAcceptSocket(ar);
        }
        catch (Exception e)
        {
            newState = new SocketState(serverTuple.Item1, e.Message);
            newState.OnNetworkAction(newState);
            return;
        }

        newState = new SocketState(serverTuple.Item1, newSocket);
        newState.OnNetworkAction(newState);
    ...
    public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
    {
        ...
        try
        {
            ...
            if (!foundIPV4)
            {
                SocketState state = new SocketState(toCall, "No IPV4 addresses found.");
        ...
        catch (Exception)
        {
            ...
            {
                SocketState state = new SocketState(toCall, "Invalid host address.");
                state.OnNetworkAction(state);
                return;
            }
        }
        ...
        try
        {
            IAsyncResult result = newState.TheSocket.BeginConnect(ipAddress, port, ConnectedCallback, newState);
            bool success = result.AsyncWaitHandle.WaitOne(3000, true);

            if (!newState.TheSocket.Connected)
            {
        ...
        {
            newState.ErrorOccurred = true;
            newState.ErrorMessage = "Error occurred when connecting to the server: ";
            newState.OnNetworkAction(newState);
            return;
        }
    ...
    private static void ConnectedCallback(IAsyncResult ar)
    {
        ...
        catch (Exception)
        {
            ...
            state.ErrorMessage = "Error occurred when connecting to the server: ";
            state.OnNetworkAction(state);
            return;
        }
    }
    ...
    public static void GetData(SocketState state)
    {
        ...
        {
            state.ErrorOccurred = true;
            state.ErrorMessage = "Error occurred when attetempting to receive data:";
            state.OnNetworkAction(state);
            return;
        }
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        try
        {
            ...
            if (bytes > 0)
            {
                //string received = Encoding.UTF8.GetString(state.buffer, 0, bytes);
                lock (state.data)
                {
                    state.data.Append(Encoding.UTF8.GetString(state.buffer, 0, bytes));
                }
                state.OnNetworkAction(state);
                return;
            }
        ...
        {
            state.ErrorOccurred = true;
            state.ErrorMessage = "There was an error when receiving data";
            state.OnNetworkAction(state);
            return;
